...
import { useCallback, useMemo, useEffect, Fragment, createContext, useContext, useRef, useState } from 'react';
...
var styles_group$2 = {"base":"GroupRow-module__base___b241-","label":"GroupRow-module__label___Du57P","group":"GroupRow-module__group___V7xa6","groupItem":"GroupRow-module__groupItem___yNSX8","x2":"GroupRow-module__x2___9bLae","last":"GroupRow-module__last___ArsSn"};

const GroupRow = ({ children, label, classNames, ...restProps }) => {
    return (jsxs("div", { ...restProps, className: cx(styles_group$2.base, classNames), children: [label && jsx("div", { className: styles_group$2.label, children: label }), jsx("div", { className: styles_group$2.group, children: children })] }));
};
...
const Input = ({ onChange, disabled, className, max, ...restProps }) => {
    ...
    return (jsx("input", { ...restProps, disabled: disabled, onChange: handleChange, readOnly: disabled, className: cx(styles$4.base, className, {
...
const Switch = ({ enabled, onSwitch, disabled, currency, isPlaying, }) => {
    const switcherClassName = useMemo(() => cx(styles$3.switcher, styles$3[currency], {
    ...
    return (jsx(Switch$1, { checked: enabled, onChange: onSwitch, as: Fragment, disabled: isPlaying, children: jsxs("div", { className: styles$3.base, children: [jsx("span", { className: styles$3.label, children: "Auto" }), jsx("div", { className: switcherClassName, children: jsx("span", { className: cx(styles$3.thumb, { [styles$3["move-right"]]: enabled }) }) })] }) }));
};

var styles$2 = {"base":"InputWithIcon-module__base___FwgXx","icon":"InputWithIcon-module__icon___KtOjW","input":"InputWithIcon-module__input___mQFr9","inputWithLabel":"InputWithIcon-module__inputWithLabel___06L8W","label":"InputWithIcon-module__label___O28vw","gold":"InputWithIcon-module__gold___GlVw1","sweeps":"InputWithIcon-module__sweeps___J4JdJ"};

const InputWithIcon = ({ children, switcherConfig, disabled, currency, label, className, ...restProps }) => {
    return (jsxs("div", { className: cx(styles$2.base, className, {
            [styles$2.disabled]: switcherConfig?.disabled,
        }), children: [label && (jsx("span", { className: cx(styles$2.label, styles$2[currency]), children: label })), jsx(Input, { ...restProps, className: cx(styles$2.input, {
                    [styles$2.inputWithLabel]: label !== undefined,
                }), disabled: disabled }), switcherConfig && jsx(Switch, { ...switcherConfig }), children && jsx("div", { className: cx(styles$2.icon), children: children })] }));
};

var styles$1 = {"base":"InputWithSwitch-module__base___vjLoh","icon":"InputWithSwitch-module__icon___upeAL","input":"InputWithSwitch-module__input___K6RCD","label":"InputWithSwitch-module__label___tEjpj","gold":"InputWithSwitch-module__gold___MO4qj","sweeps":"InputWithSwitch-module__sweeps___jvyQd"};

const InputWithSwitch = ({ children, switcherConfig, disabled, currency, label, className, ...restProps }) => {
    return (jsxs("div", { className: cx(styles$1.base, className, {
            [styles$1.disabled]: switcherConfig.disabled,
        }), children: [label && (jsx("span", { className: cx(styles$1.label, styles$1[currency]), children: label })), jsx(Switch, { ...switcherConfig }), jsx(Input, { ...restProps, className: cx(styles$1.input), disabled: disabled, max: 99 }), jsx("div", { className: cx(styles$1.icon), children: children })] }));
};
...
var style_chevron = {"chevron":"ChevronIcon-module__chevron___lif8s","open":"ChevronIcon-module__open___niD1n"};
...
const usePlayController = (side) => {
    const { config, mode, autoPlay: { setNumberOfPlays, numberOfPlays, setPlayedRounds, playedRounds, setState, state, }, } = useAutoManualPlayState();
    const { currentCurrency, currencies } = config.currencyOptions;
    const { isPlaying, canCashout, disabledController, disabledMenu, autoPlayDelay = 1000, playHook, lastPlayedSide, } = config.playOptions;
    const { leftPlayAmount, rightPlayAmount, setLeftBetAmount, setRightBetAmount, playLimits, } = playHook?.() ?? {};
    ...
    const setPlayAmount = side === PlaySide.LEFT ? setLeftBetAmount : setRightBetAmount;
    const stopAutoplay = () => {
    ...
    const loopRounds = (currentPlayedRounds, remainingPlays, playSide) => {
        ...
        config.onAutoPlay(() => {
            ...
            playIntervalRef.current = setTimeout(() => loopRounds(currentPlayedRounds + 1, remainingPlays - 1, playSide), autoPlayDelay);
        }, stopAutoplay, playSide);
    };
    const handleAutoPlay = (playSide) => {
    ...
    return {
        ...
        manualPlay: {
            isDisabled,
            onPlay: (playSide) => config.onPlay(playSide),
            canCashout,
        },
        autoPlay: {
...
const PlayAmountControl = ({ playAmount, minPlayAmount, maxPlayAmount, isDisabled, adjustPlayAmount, onChangeAmount, onBlurAmount, currentCurrency, currencies, disableInput, }) => {
    return (jsxs(GroupRow, { children: [jsx(InputWithIcon, { className: styles_group$2.groupItem, value: playAmount, type: "number", onChange: onChangeAmount, onBlur: onBlurAmount, placeholder: minPlayAmount.toString(), max: maxPlayAmount, min: minPlayAmount, disabled: isDisabled() || disableInput, currency: currentCurrency, label: "Play Amount", children: jsx(SelectMenu, { currencies: currencies, selectedCurrency: currentCurrency, setSelectedCurrency: sendSetUserCurrencyEvent, disabled: isDisabled() }) }), jsx(Button, { className: styles_group$2.groupItem, onClick: () => adjustPlayAmount(PLAY_HALVE), theme: "ghost", disabled: isDisabled(), children: jsx("span", { className: styles_group$2.x2, children: "-" }) }), jsx(Button, { className: styles_group$2.groupItem, onClick: () => adjustPlayAmount(PLAY_DOUBLE), theme: "ghost", disabled: isDisabled(), children: jsx("span", { className: cx(styles_group$2.x2, styles_group$2.last), children: "+" }) })] }));
};
...
const selectButton = (gameMode, side = PlaySide.LEFT) => {
...
const addPressedClass = (gameMode, activeClassName, side) => {
...
const removePressedClass = (gameMode, activeClassName, side) => {
    const button = selectButton(gameMode, side);
    if (!button) {
        return;
    }
...
const AutoPlayController = ({ side = PlaySide.LEFT }) => {
    ...
    return (jsxs(Fragment$1, { children: [jsx(PlayAmountControl, { playAmount: playAmount, minPlayAmount: minPlayAmount, maxPlayAmount: maxPlayAmount, isDisabled: isDisabled, adjustPlayAmount: adjustPlayAmount, onChangeAmount: onChangeAmount, onBlurAmount: onBlurAmount, currentCurrency: currentCurrency, currencies: currencies, disableInput: playOptions.disableInput }), jsx(Button, { disabled: isButtonDisabled, className: getClassName, onClick: () => buttonAction(side), roleType: roleButton, children: buttonLabel })] }));
};

var styles = {"base":"MiniInputWithIcon-module__base___OJi-U","label":"MiniInputWithIcon-module__label___GdpSt","inputGroup":"MiniInputWithIcon-module__inputGroup___RU7N4","icon":"MiniInputWithIcon-module__icon___xV9Gs","input":"MiniInputWithIcon-module__input___PSu7q","gold":"MiniInputWithIcon-module__gold___MmROS","sweeps":"MiniInputWithIcon-module__sweeps___Ng-SO"};

const MiniInputWithIcon = ({ children, disabled, currency, label, className, ...restProps }) => {
    return (jsxs("div", { className: cx(styles.base, className), children: [jsx("span", { className: cx(styles.label, styles[currency]), children: label }), jsxs("div", { className: cx(styles.inputGroup), children: [children && jsx("div", { className: cx(styles.icon), children: children }), jsx(Input, { ...restProps, className: cx(styles.input, {
                            [styles.inputWithLabel]: label !== undefined,
                        }), disabled: disabled })] })] }));
};

var styles_group$1 = {"base":"MiniPlayAmountController-module__base___5Qd6W","group":"MiniPlayAmountController-module__group___sKWqB","x2":"MiniPlayAmountController-module__x2___jTHPf","first":"MiniPlayAmountController-module__first___zcdE1","last":"MiniPlayAmountController-module__last___pAYtL"};
...
const MiniPlayAmountControl = ({ playAmount, minPlayAmount, maxPlayAmount, isDisabled, adjustPlayAmount, onChangeAmount, onBlurAmount, currentCurrency, currencies, disableInput, }) => {
    return (jsxs("div", { className: cx(styles_group$1.base), children: [jsxs("div", { className: cx(styles_group$1.group), children: [jsx(Button, { className: styles_group$1.groupItem, onClick: () => adjustPlayAmount(PLAY_HALVE), theme: "ghost", disabled: isDisabled(), children: jsx("span", { className: cx(styles_group$1.x2, styles_group$1.first), children: "-" }) }), jsx(Button, { className: styles_group$1.groupItem, onClick: () => adjustPlayAmount(PLAY_DOUBLE), theme: "ghost", disabled: isDisabled(), children: jsx("span", { className: cx(styles_group$1.x2, styles_group$1.last), children: "+" }) })] }), jsx(MiniInputWithIcon, { className: styles_group$1.groupItem, value: playAmount, type: "number", onChange: onChangeAmount, onBlur: onBlurAmount, placeholder: minPlayAmount.toString(), max: maxPlayAmount, min: minPlayAmount, disabled: isDisabled() || disableInput, currency: currentCurrency, label: "Play Amount", children: jsx(SelectMenu, { currencies: currencies, selectedCurrency: currentCurrency, setSelectedCurrency: sendSetUserCurrencyEvent, disabled: isDisabled() }) })] }));
};

var styles_group = {"base":"ManualPlayController-module__base___4llBv"};

const ManualPlayController = ({ side = PlaySide.LEFT, lastPlayedSide, }) => {
    const { currentCurrency, currencies, playAmount, playOptions, minPlayAmount, maxPlayAmount, isValidPlayAmount, adjustPlayAmount, onChangeAmount, onBlurAmount, manualPlay: { isDisabled, onPlay }, } = usePlayController(side);
    ...
    return (jsxs("div", { className: cx(styles_group.base), children: [jsx(MiniPlayAmountControl, { playAmount: playAmount, minPlayAmount: minPlayAmount, maxPlayAmount: maxPlayAmount, isDisabled: isDisabled, adjustPlayAmount: adjustPlayAmount, onChangeAmount: onChangeAmount, onBlurAmount: onBlurAmount, currentCurrency: currentCurrency, currencies: currencies, disableInput: playOptions.disableInput }), jsx(Button, { disabled: isButtonDisabled, className: cx({
                ...
                }), onClick: () => onPlay(side), roleType: roleButton, children: "Play" })] }));
};

var style_multi = {"base":"ManualMultiPlayController-module__base___RUe4A"};

const ManualMultiPlayController = ({ lastPlayedSide, }) => {
    return (jsxs("div", { className: cx(style_multi.base), children: [jsx(ManualPlayController, { side: PlaySide.LEFT, lastPlayedSide: lastPlayedSide }), jsx(ManualPlayController, { side: PlaySide.RIGHT, lastPlayedSide: lastPlayedSide })] }));
};

var styles_ui = {"base":"UI-module__base___wThyQ","betForm":"UI-module__betForm___hQkYd","multiPlayController":"UI-module__multiPlayController___p-8ht","disabled":"UI-module__disabled___dnZJX","auto":"UI-module__auto___5peb8"};
...
const Selector = ({ currentValue, label, riskColor, values, onSelect, disabled, }) => {
    ...
    return (jsxs("div", { className: cx(style_select.base), children: [label && jsx("span", { className: cx(style_select.label), children: label }), jsx(Menu, { as: "div", className: cx(style_select.menu), children: ({ open }) => (jsxs(Fragment$1, { children: [jsxs(MenuButton, { className: cx(style_select.button, {
...
const AutoManualPlayProvider = ({ children, config, }) => {
    ...
    return (jsxs(AutoManualPlayStateContext.Provider, { value: contextValue, children: [typeof children === "function" ? children(contextValue) : children, config.playOptions.displayController && (jsxs("div", { className: cx(styles_ui.base, styles_ui.betForm, {
                    ...
                    "--play-dropdown-bg": hexToRgb(config.dropdown.bgColorHex ?? "#01243A"),
                                    ...
                                    disabled: config.playOptions.disabledController ||
                                        ...
                                        [styles_ui.disabled]: mode !== GAME_MODE.AUTOPLAY ||
                                            ...
                                            autoplayState === AUTO_PLAY_STATE.PLAYING,
                                    }), children: `∞` }) })] }), mode === GAME_MODE.MANUAL ? (jsx(ManualMultiPlayController, { lastPlayedSide: config.playOptions.lastPlayedSide })) : (jsx(AutoPlayController, {}))] }))] }));
};
